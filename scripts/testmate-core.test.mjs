...
const schema = {
    ...
    properties: {
        ...
        auditLogPath: { type: 'string', pattern: '^logs\\/(pre_commit|pre_mr|pre_merge|pre_release)_[^\\/]+\\.md$' }
    }
};
...
test('sanitizes and falls back for invalid audit log paths', () => {
    ...
    assert.equal(resolved, 'logs\\pre_mr_2026-04-29T12-00-00.md');
    assert.throws(() => resolveAuditLogPath('logs/custom.md', 'tier-2-impact'), /formal modes/);
});

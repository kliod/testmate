...
export function validateQualityGateOutput(data, schema, {
    ...
    if (typeof data.auditLogPath === 'string' && !data.auditLogPath.startsWith(`.testmate/logs/${expectedMode}_`)) {
        errors.push(`Invalid auditLogPath for ${expectedMode}: expected .testmate/logs/${expectedMode}_<timestamp>.md, received ${data.auditLogPath}`);
    }
...
export function createAuditLogPath(formalMode, now = new Date()) {
    ...
    return path.join('.testmate', 'logs', `${formalMode}_${timestamp}.md`).replace(/\\/g, '/');
}
...
export function resolveAuditLogPath(parsedAuditLogPath, formalMode, now = new Date()) {
    ...
    return path.join('.testmate', 'logs', filename);
}
